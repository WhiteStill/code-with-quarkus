...
DB 사용자 체크를 추가하였다.
로그인 후 페이지를 추가하였다.
/logout 로그아웃 엔드포인트를 추가하였다.
로그인 폴더에 login.html에 로그인버튼 아래에 회원가입 버튼을 추가하였다.
register 엔드포인트 등록을 하였다.
회원가입 화면 작성하기 위해 register.html를 추가하여 기존에 있던 디자인을 재활용하여 추가하였다.
마지막으로 기존 유저 자바에 있는 내용을 수정하여 회원 테이블을 추가하였다
입력 값 유효성 검사(JS)를 추가하여 정확하게 입력하지 않으면 넘어가지 않게 추가하였다.
SHA-256 해시, 모달창을 추가하였다. (패스워드 해시 암호화 후 확인 모달 출력)
<div align="center">
<img src="screenshots\스크린샷 2026-05-13 141341.png"실습8 화면">
</div>
<br>
...
## 12주차수업내용
login 폴더 : profile.html를 추가하였다.
uploads/profile/ 을 추가하였다.
로그인 페이지 암호화 구현을 위해 login.html 파일을 수정하였다.
login.js 파일을 유효성 검사로 수정하였다.
암호화 체크를 수행하였다.
메인 화면 세션 체크하는 코드를 AuthResource.java에 추가하였다.
그리고 네비바 프로필을 링크를 추가하였다.
추가적으로 
function validateAndLogin() {
    let valid = true;
